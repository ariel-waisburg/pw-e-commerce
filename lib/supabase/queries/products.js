...
import { createSupabaseServerClient, getSupabaseServiceRole } from "@/lib/supabase/server";
import { getStorageMediaRef } from "@/lib/products/media";
...
async function attachStorageMedia(products = []) {
...
async function fetchProducts(queryBuilder) {
  ...
  return attachStorageMedia(normalized);
}
