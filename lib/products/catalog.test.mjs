...
test("filterCatalogProducts matches any product whose saleType is in a multi-value filter array", () => {
  const mattress = { id: "p-mattress", saleType: "mattress", variants: [{ id: "v1", price: 100000, stockQuantity: 1 }] };
  const set = { id: "p-set", saleType: "set", variants: [{ id: "v2", price: 100000, stockQuantity: 1 }] };

  const filtered = filterCatalogProducts([mattress, set], { saleType: ["mattress", "set"] });

  assert.deepEqual(filtered.map((product) => product.id).sort(), ["p-mattress", "p-set"]);
});

test("filterCatalogProducts matches any variant whose measureCode is in a multi-value measures filter", () => {
  const product = {
    id: "p-multi-measure",
    saleType: "mattress",
    variants: [
      { id: "v-160", price: 100000, stockQuantity: 1, measureCode: "160x200" },
      { id: "v-140", price: 90000, stockQuantity: 1, measureCode: "140x190" },
      { id: "v-90", price: 70000, stockQuantity: 1, measureCode: "90x190" },
    ],
  };

  const filtered = filterCatalogProducts([product], { measures: ["160x200", "140x190"] });
...
test("collectCatalogFilterOptions exposes main and secondary filter groups", () => {
  ...
  assert.deepEqual(options.firmnesses.map((option) => option.value), ["balanced"]);
  assert.deepEqual(options.motionIsolationLevels.map((option) => option.value), ["high", "low"]);
