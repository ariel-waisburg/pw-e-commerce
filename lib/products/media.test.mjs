...
test("getProductImageFilename maps core catalog products to the expected filenames", () => {
  assert.equal(
    getProductImageFilename({ line: "Classic Rest", technology: "pocket", saleType: "mattress" }),
    "CLASSIC-POCKET-1.jpg"
  );
  assert.equal(
    getProductImageFilename({ line: "Classic Rest", technology: "pocket", saleType: "set" }),
    "CLASSIC-POCKET.jpg"
  );
  assert.equal(
...
test("storage and local refs derive from the same filename mapping", () => {
  ...
  assert.equal(storageRef.path, "HIGH-REST-MID.jpg");
  assert.equal(localRef.url, "/products/HIGH-REST-MID.jpg");
...
test("card media candidates prefer remote media and fall back to catalog media without duplicates", () => {
  ...
  assert.equal(catalogMedia[0]?.url, "/products/SUPERIOR-REST-ULTRA-PLUSH.jpg");
  assert.equal(candidates[0]?.url, "https://cdn.sleep.test/superior-rest-pocket-set.jpg");
  assert.equal(candidates[1]?.url, "/products/SUPERIOR-REST-ULTRA-PLUSH.jpg");
