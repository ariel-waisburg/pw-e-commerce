...
test("normalizeStorageMediaUrl maps legacy bucket filenames to current local product files", () => {
  assert.equal(
    normalizeStorageMediaUrl("storage://colchones/HIGH-REST-PLUSH-1.jpg", {
    ...
    "/products/HIGH-REST-PLUSH.jpg"
  );

  assert.equal(
    normalizeStorageMediaUrl("storage://colchones/CLASSIC-REST-BONELL.jpg", {
      slug: "conjunto-classic-rest-bonell",
      name: "Conjunto Classic Rest Bonell",
      line: "Classic Rest",
      technology: "bonell",
      saleType: "set",
    }),
    "/products/CLASSIC-REST-BONELL-S.jpg"
  );
});

test("normalizeStorageMediaUrl preserves non-storage URLs", () => {
  assert.equal(
    normalizeStorageMediaUrl("https://cdn.sleep.test/high-rest.jpg", {
      line: "High Rest",
      technology: "bonell",
      saleType: "mattress",
    }),
    "https://cdn.sleep.test/high-rest.jpg"
  );
});
