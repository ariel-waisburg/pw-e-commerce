...
const STORAGE_URL_PREFIX = `storage://${STORAGE_BUCKET}/`;
...
export function normalizeStorageMediaUrl(url, product = {}) {
  if (typeof url !== "string" || !url.startsWith(STORAGE_URL_PREFIX)) {
    return url;
  }
  ...
  const storagePath = url.slice(STORAGE_URL_PREFIX.length).replace(/-1(\.[^.]+)$/i, "$1");
  return `/products/${storagePath}`;
}
