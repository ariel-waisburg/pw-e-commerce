...
export const HOMEPAGE_QUICK_LINKS = [
  {
    label: "Solo colchón",
    href: "/catalog?saleType=mattress",
    description: "Compará modelos sin sumar el sommier.",
  },
  {
    label: "Colchón + sommier",
...
function hasProductMedia(product = {}) {
...
export function selectHomepagePreviewProducts(products = [], limit = HOMEPAGE_PREVIEW_LIMIT) {
  const mattresses = products.filter((product) => isMattressProduct(product) && hasProductMedia(product)).sort(compareHomepageProducts);
  const selected = [];
