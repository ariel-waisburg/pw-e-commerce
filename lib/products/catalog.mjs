...
export const PLAZA_ORDER = ["1 plaza", "1 plaza y media", "2 plazas", "queen", "king"];
...
export function formatMeasureLabel(measureCode) {
  const digits = String(measureCode ?? "").match(/(\d{2,3})\D+(\d{3})/);
  if (!digits) return "";
  const width = digits[1].padStart(3, "0");
  const length = digits[2];
...
export function normalizeCatalogProduct(product = {}) {
  ...
  if (line && technology && !derived.isTechnologyAllowed) {
    console.warn(
      `[catalog] Invalid line/technology combination "${line}" + "${technology}" for product ${product.slug ?? product.id ?? product.name ?? "unknown"}`
    );
  }
...
export function filterCatalogProducts(products = [], filters = {}) {
  const saleTypeList = toFilterList(filters.saleType).length
    ? toFilterList(filters.saleType)
    : toFilterList(filters.type === "colchon" ? "mattress" : filters.type === "conjunto" ? "set" : null);
  ...
  const normalizedFilters = {
    ...
    measures: toFilterList(filters.measures).map((code) => normalizeMeasureCode(code)).filter(Boolean),
  ...
  return products.filter((product) => {
    if (normalizedFilters.saleType.length && !normalizedFilters.saleType.includes(product.saleType)) return false;
    ...
    if (!product.variants.length) {
      const hasVariantLevelFilter = Boolean(
        normalizedFilters.measure ||
          normalizedFilters.measures.length ||
    ...
    return product.variants.some((variant) => {
      ...
      if (normalizedFilters.measures.length && !normalizedFilters.measures.includes(variant.measureCode)) return false;
...
export function applyVariantFiltersToProduct(product, filters = {}) {
  ...
  const normalizedMeasures = toFilterList(filters.measures).map((code) => normalizeMeasureCode(code)).filter(Boolean);
  ...
  const visibleVariants = product.variants.filter((variant) => {
    ...
    if (normalizedMeasures.length && !normalizedMeasures.includes(variant.measureCode)) return false;
