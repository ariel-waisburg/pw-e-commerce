...
const ALIAS_LOOKUP = SIZE_DEFINITIONS.reduce((accumulator, definition) => {
  ...
  const comparableLabel = toComparable(definition.label);
  if (!accumulator.has(comparableLabel)) {
    accumulator.set(comparableLabel, definition.measureCode);
  }
  const comparableName = toComparable(definition.name);
  if (!accumulator.has(comparableName)) {
    accumulator.set(comparableName, definition.measureCode);
  }
  return accumulator;
}, new Map());
