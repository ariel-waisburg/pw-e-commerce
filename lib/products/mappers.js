...
function mapCatalogProductToCardData(product, rawProduct = {}) {
  ...
  const media = getDerivedProductMedia(mediaProduct);
  const catalogMedia = getCatalogMedia(product);
  const mediaCandidates = getCardMediaCandidates(mediaProduct);
...
function mapCatalogProductToDetailData(product, rawProduct = {}) {
  const primaryVariant = product.variants[0] ?? null;

  const media = getDerivedProductMedia({
    ...product,
    media: (rawProduct.media ?? []).map((asset) => ({
      id: asset.id,
      url: asset.url,
