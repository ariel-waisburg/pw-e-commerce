...
test("catalog state parses comma-separated saleType and measures as arrays, with a default of empty", () => {
  const state = getCatalogStateFromSearchParams({
    saleType: "mattress,set",
    measures: "160x200,140x190",
  });

  assert.deepEqual(state.saleType, ["mattress", "set"]);
  assert.deepEqual(state.measures, ["160x200", "140x190"]);

  const emptyState = getCatalogStateFromSearchParams({});
  assert.deepEqual(emptyState.saleType, []);
  assert.deepEqual(emptyState.measures, []);
});

test("legacy single-value saleType and type params still resolve to a one-item array", () => {
  const fromSaleType = getCatalogStateFromSearchParams({ saleType: "mattress" });
  assert.deepEqual(fromSaleType.saleType, ["mattress"]);

  const fromLegacyType = getCatalogStateFromSearchParams({ type: "conjunto" });
  assert.deepEqual(fromLegacyType.saleType, ["set"]);
});

test("catalog discovery filters by multiple saleType and measures values at once", () => {
  const model = buildCatalogDiscoveryModel(products, {
    saleType: "mattress,set",
    measures: "160x200",
  });

  assert.ok(model.visibleProducts.length > 0);
  assert.ok(model.visibleProducts.every((product) => ["mattress", "set"].includes(product.saleType)));
  assert.ok(
    model.visibleProducts.every((product) =>
      product.variants.some((variant) => variant.measureCode === "160x200")
    )
  );

  const saleTypeChips = model.activeFilterChips.filter((chip) => chip.key === "saleType");
  assert.deepEqual(saleTypeChips.map((chip) => chip.value).sort(), ["mattress", "set"]);
});
