...
test("mapFallbackProducts exposes the new mattress catalog domain on cards", () => {
  ...
  assert.equal(card.displayName, "Mid");
  ...
  assert.equal(card.media[0]?.url, "/products/HIGH-REST-MID-1.jpg");
  assert.equal(card.catalogMedia[0]?.url, "/products/HIGH-REST-MID-1.jpg");
  assert.equal(card.mediaCandidates[0]?.url, "/products/HIGH-REST-MID-1.jpg");
});
...
test("mapSupabaseProductToCardData keeps mattress/set cards in the main domain and exposes remote to local media fallback", () => {
  ...
  assert.equal(card.catalogMedia[0]?.url, "/products/SUPERIOR-REST-ULTRA-PLUSH.jpg");
  assert.equal(card.mediaCandidates[0]?.url, "https://cdn.sleep.test/catalog/superior-pocket-set.jpg");
  assert.equal(card.mediaCandidates[1]?.url, "/products/SUPERIOR-REST-ULTRA-PLUSH.jpg");
...
test("mapFallbackProductToDetail builds the new technical summary and set split data", () => {
  ...
  assert.equal(detail.media[0]?.url, "/products/TOP-HOTEL-REST.jpg");
